...
export function AnexosStep({ onPrevious }: AnexosStepProps) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {CUADRANTES.map((cuadrante) => (
            <Controller
              ...
              name={cuadrante.key}
              control={control}
              rules={{ required: `${cuadrante.title} es un campo obligatorio` }}
      ...
      <Field>
        ...
        <p className="text-xs text-muted-foreground">
          Se genera automáticamente a partir de los pasos del Modelo de negocio
          ya completados.
        </p>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {canvas.map((bloque) => (
            <div key={bloque.id} className="rounded-lg border bg-muted/40 p-3">
