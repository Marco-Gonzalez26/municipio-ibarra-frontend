...
import { AsistenciaTable } from '@/features/formularios/components/asistencia-table'
...
export default async function AsistenciaPage({
  ...
  const [formulariosRes, emprendedoresRes] = await withSessionRedirect(() =>
    Promise.all([
      entrepeneurFormService.getAllAsistenciaTecnica(
        page,
        LIMIT,
        session.token
      ),
  ...
  return (
    <>
      <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4 ">
        ...
        <h1 className="text-sm font-medium">Asistencia Técnica</h1>
      </header>
